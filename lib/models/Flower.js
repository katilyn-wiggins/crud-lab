...
module.exports = class Flower {
    ...
    static async allFlowersWithId(id) {
        const {rows} = await pool.query("SELECT * from flowers WHERE id=$1", [id]);
        return rows.map((row) => new Flower(row));
    }
    ...
    static async deleteAFlower(id) {
        const {rows} = await pool.query("DELETE from flowers WHERE id=$1 RETURNING *", [
            id
        ]);
        return new Flower(rows[0]);
    }
